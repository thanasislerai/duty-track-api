...
const isWeekDay = (day: string) =>
    Object.keys(Day).map(k => Day[k]).includes(day)
...
export const dutySaveErrorHandler = (
    ...
    if (error.sqlMessage?.includes('weekly_on_chk')) {
        ...
        if (dto.frequency === DutyFrequency.WEEKLY && !dto.weeklyOn) {
            console.log('Hello')
            throw new BadRequestException('You need to specify a week day for a weekly task')
        }
    }

    if (!isWeekDay(dto.weeklyOn) && (dto.frequency === DutyFrequency.WEEKLY || !!dto.weeklyOn)) {
